...
docker compose up -d zookeeper activemq-artemis redis mongodb cassandra elasticsearch logstash kibana jaeger
...
# Create all required topics including DLQ variants
...
# ── 3a. Start fraud model server (Python/LightGBM) ────────────────────────────
echo ""
echo "[2b/4] Starting fraud model server (LightGBM on PaySim)..."
if python3 -c "import lightgbm, fastapi, uvicorn" &>/dev/null 2>&1; then
  if [ -f "$LOGS/fraud-model.pid" ]; then
    old_pid=$(cat "$LOGS/fraud-model.pid")
    kill "$old_pid" 2>/dev/null || true
  fi
  nohup python3 "$BASE/fraud_model_server.py" \
    > "$LOGS/fraud-model.log" 2>&1 &
  echo $! > "$LOGS/fraud-model.pid"
  echo "      Fraud model server started on :8091 (training in background)"
  echo "      Tail log: $LOGS/fraud-model.log"
else
  echo "      ⚠️  lightgbm/fastapi not installed — fraud scoring uses heuristic fallback"
  echo "      Install: pip install -r $BASE/requirements_fraud_model.txt"
fi
...
# ── 4. Health check (polls each service, up to 90s per service) ───────────────
echo ""
echo "[4/4] Waiting for services to become healthy (max 90s each)..."
...
# Also report fraud model server
model_code=$(curl -s -o /dev/null -w "%{http_code}" \
  "http://localhost:8091/health" --connect-timeout 2 2>/dev/null || echo "ERR")
if [ "$model_code" = "200" ]; then
  echo "  ✓ fraud-model-server (:8091) — LightGBM ready"
else
  echo "  ⏳ fraud-model-server (:8091) — still training (check $LOGS/fraud-model.log)"
fi
...
if $all_up; then
  ...
  echo ""
  echo "  Common fixes:"
  echo "    Port clash  → pkill -f '\\.jar' && bash $BASE/start_live_traffic.sh"
