#!/usr/bin/env python3
"""GALA-inspired baseline, FULL 97-case benchmark, MEMORY_ENABLED=False.
The real, matched ablation counterpart to agentic_full_benchmark.py."""
import json
import os
import time
...
RESULTS_FILE = "gala_inspired_full_results.json"
...
def main():
    ...
    print(f"GALA-inspired full run: {len(remaining)} cases left of {len(targets)} confirmed total.", flush=True)
    ...
    n = len(targets)
    print(f"\nGALA-inspired (no memory) full result: literal {hits_literal}/{n} = {hits_literal/n*100:.1f}%  "
