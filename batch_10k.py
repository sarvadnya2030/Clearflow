#!/usr/bin/env python3
"""ClearFlow 10K payment test — 10 batches × 1000"""
...
def get_token():
    r = requests.post(f"{GATEWAY}/auth/token",
        json={"clientId":"clearflow-demo","clientSecret":"demo-secret-2024","grantType":"client_credentials"}, timeout=5)
...
start = time.time()
print(f"10K Payment Test — {NUM_BATCHES} batches × {BATCH_SIZE}")
print(f"{'Batch':>7} | {'Accept%':>8} | {'Sent':>7}")
